 ...
 * 查看确认信息
 */
public class LookQrServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("msg", "您尚未登录, 请登录, 错误代码:10071");
		...
		try {
			...
			e.printStackTrace();
			request.setAttribute("msg", "服务器发生错误, 错误代码:10072");
		...
		if (user == null || !(user.getUsername().equals("admin"))) {
			// 不是管理员
			request.setAttribute("msg", "抱歉, 您不是管理员, 错误代码:10073");
		...
		try {
			list = new FormService().queryQr();
		} catch (SQLException e) {
			e.printStackTrace();
		}
		request.setAttribute("list", list);
		request.getRequestDispatcher("/lookQr.jsp").forward(request, response);
	}
